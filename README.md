...
- 캘린더 검진 일정 — 국가 영유아 건강검진·구강검진 일정을 생년월일 기준으로 계산해서 해당 날짜 칸에 점으로 표시하고, 달력 아래 "이번 달 검진" 목록에서 완료 체크를 할 수 있어요.
- 예방접종 일정 (홈 탭에서 진입) — 질병관리청 표준예방접종일정표 기준으로 생년월일에 맞춰 접종 시기를 계산하고, 같은 날 맞는 접종끼리 묶어서 보여줘요. 완료 체크는 가족 모두에게 공유돼요.
- 발달 체크 (홈 탭에서 진입) — 대근육은 WHO 운동발달 기준, 나머지(소근육·언어·사회정서·인지)는 월령별 체크리스트로 확인해요. 완료 체크는 가족 모두에게 공유돼요.
- 오늘의 스케줄표 (홈 탭에서 진입) — 아이 등록 시 만들어진 기본 시간표(기상·간식·낮잠·밤잠)를 보고 실제 한 시간을 기록해요. 예정 시간과 실제 시간의 차이(+/-분)도 보여줘요. "관리" 모드에서 항목을 추가·삭제할 수 있어요.
- 육아일기 (홈 탭에서 진입) — 날짜별로 하루 한 장 사진과 짧은 글을 남겨요. 캘린더 탭의 날짜별 사진과 같은 사진이라 어느 쪽에서 올려도 서로 반영돼요. 가족 모두가 같은 일기를 보고 이어 쓸 수 있어요.
- 할 일 목록 (홈 탭에서 진입) — 가족 단위 체크리스트. 추가·완료·삭제가 실시간으로 공유돼요.
- 다크모드 — 내정보 탭에서 라이트/다크/시스템 중 고를 수 있어요. 기기별로 저장되는 개인 설정이라 가족 구성원마다 다르게 쓸 수 있어요.

원래 계획했던 화면과 기능은 모두 이 앱으로 옮겨졌어요. 이제부터는 실제 사용해보면서 다듬는 단계예요.
